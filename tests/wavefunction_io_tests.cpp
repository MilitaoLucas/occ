...
fs::path test_directory = fs::path(occ::get_data_directory()) / "test_files";
TEST_CASE("Read H2 fchk contents", "[read]") {
  occ::io::FchkReader reader((test_directory / "common_files" / "fchk_contents.fchk").string());
...
TEST_CASE("Read water UHF fchk file", "[read]") {
  occ::io::FchkReader reader((test_directory / "common_files" / "unrestricted_fchk_contents.fchk").string());
...
TEST_CASE("Read Gaussian09 water fchk spherical basis", "[read]") {
  occ::io::FchkReader reader((test_directory / "wavefunction_io" / "pure_fchk_contents.fchk").string());
  REQUIRE(reader.num_alpha() == 5);
...
void check_wavefunctions(const occ::qm::Wavefunction &wfn,
  ...
  auto check_same_coeffs = [](const auto &c1, const auto &c2) {
    for (Eigen::Index i = 0; i < c1.rows(); i++) {
      for (Eigen::Index j = 0; j < c1.cols(); j++) {
...
TEST_CASE("Read molden output formamide molecule", "[read]") {
  occ::io::MoldenReader reader((test_directory / "wavefunction_io" / "molden_contents.molden").string());
